 ...
 * 画面に出すだけでは手で打ち直す手間だけど、Macに最初から入っている pbcopy というコマンドを、
 * Javaから呼び出して代わりに働いてもらう。
 * ProcessBuilder は「別のプログラムを起動して、その入力に文字を流し込む」ための標準クラス。
 * 失敗しても致命的ではないから、成功／失敗を boolean で返す方針に。
 */
public class ClipboardUtil {
    ...
    public static boolean copy(String text) {
        try {
            ...
            return exitCode == 0;
            // pbcopy の終了を待ち、終了コード0（正常終了）だけを成功に
        } catch (IOException e) {
            return false;
            // pbcopy が無い環境（Windowsなど）では起動自体に失敗する想定なので、その場合は静かに失敗を返す
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            return false;
            // 待っている間に中断された場合は、中断された印を戻してから失敗を返すのが作法らしい...
        }
    }
}
